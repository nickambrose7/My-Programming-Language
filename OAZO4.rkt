...
(struct LibFuncVOneToOne ([func : (Value -> Value)]) #:transparent)
(struct LibFuncVZero ([func : (-> Value)]) #:transparent)
(struct LibFuncVCombo ([func : ((Listof Value) -> Value)]) #:transparent)
(struct ErrorV ([func : (Value -> Void)]) #:transparent)

(define-type Value (U NumV BoolV StrV ClosV PrimV PrimVAny LibFuncVZero LibFuncVOneToOne LibFuncVCombo ErrorV))
...
; This function takes a string and prints it out
(define (println [s : Value]) : Value
  (if (StrV? s)
      (begin
        (displayln (StrV-s s))
        (BoolV #t))
  (error 'println "OAZO: String not given for println")))

(check-equal? (println (StrV "Hello world (also enter 8 for the next test)")) (BoolV #t))
(check-exn (regexp "OAZO: String not given for println") (lambda () (println (NumV 5))))

; This function reads a line of input from the terminal and returns it
      ...
      (match input
        [(? string? s)
         (match (string->number s) ;
           [(? real? n) (NumV n)]
           [_ (error 'read-num "OAZO: Did not get a valid number from input")])]
        [eof (NumV 0)]))))  ; Explicitly handle EOF

(check-equal? (read-num) (NumV 8))
(displayln "Just press enter for the next input")
(check-exn (regexp "OAZO: Did not get a valid number from input") (lambda () (read-num)))

; This function joins together all arguemnts into a single string
(define (++ [args : (Listof Value)]) : Value
  (let ([serialized (map (lambda ([arg : Value])
                           (if (StrV? arg)
                               (StrV-s arg)
                               (serialize arg))) ; Otherwise, apply serialize
                         args)])
    (StrV (apply string-append serialized))))
(check-equal? 
 (++ (list (NumV 42) (StrV " is the answer, and it is ") (BoolV #t) (StrV "!")))
 (StrV "42 is the answer, and it is true!"))

; Returns the value of the last Value passed in
; The args should have already been interp'd
(define (seq [args : (Listof Value)]) : Value
  (last args))

(check-equal? (seq (list (NumV 5) (StrV "hi") (StrV "world"))) (StrV "world"))
...
(define base-env
  (Env (list (Bind '+ (PrimV prim+))
        ...
        (Bind '<= (PrimV prim<=))
        (Bind '++ (LibFuncVCombo ++))
        (Bind 'read-num (LibFuncVZero read-num))
        (Bind 'println (LibFuncVOneToOne println))
        (Bind 'seq (LibFuncVCombo seq))
...
; parse test
(check-equal? (parse '{++ "abc"}) (appC (idC '++) (list (strC "abc"))))
...
; Takes an expression and an environment and outputs a value
; by evaluating the expression and using deferred subsitution.
...
(check-equal? (interp (appC (lamC '() (numC 8)) '()) base-env) (NumV 8))
...
(check-equal? (interp (appC (lamC (list 'a)
                                  (appC (idC '+) (list (numC 2) (idC 'a)))) (list (numC 2))) base-env) 
  (NumV 4))
...
; Test lib funcs
(check-exn (regexp "OAZO: Incorrect number of arguments for library function")
           (lambda () (interp (appC (idC 'read-num) (list (numC 5))) base-env)))

(check-exn (regexp "OAZO: Incorrect number of arguments for library function")
           (lambda () (interp (appC (idC 'seq) '()) base-env)))
(check-exn (regexp "OAZO: Incorrect number of arguments for library function")
           (lambda () (interp (appC (idC 'println)
                                    (list (strC "Hi") (strC "Hello"))) base-env)))

; Test ErrorV 
(check-exn (regexp "user-error: 5")
           (lambda () (interp (appC (idC 'error) 
...
(check-equal? (top-interp '{++ "abc"}) "\"abc\"")

; Our game:
(check-equal? (top-interp
               '{seq
                 {println "Welcome to the coin flip game"}
                 {let {flip-coin <- {anon {x} :
                                          {if {<= x 5}
                                              then
                   ...
                   {let {user-input <- {read-num}}
                     {flip-coin user-input}}}}}) "true")

; Sample output from out game:
